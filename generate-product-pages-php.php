<?php
/**
 * Générateur de pages produits PHP avec includes header/footer cohérents
 */
...
foreach ($products as $product) {
    ...
    $fileName = __DIR__ . '/produit/' . $code . '.php';
    ...
    $pageContent = generateProductPage($code, $nom, $sousTitre, $descCourte, $descLongue, $categorie,
...
echo "\n🎉 $generated pages produits PHP générées !\n";

function generateProductPage($code, $nom, $sousTitre, $descCourte, $descLongue, $categorie,
                             $poids, $epaisseur, $formatMax, $usage, $dureeVie, $certification,
                             $finition, $impressionFaces, $delai, $stock, $rating, $reviewCount,
                             $prix0_10, $prix11_50, $prix51_100, $prix101_300, $prix300plus) {

    $metaDesc = "$nom pour impression grand format ✓ Prix dégressifs {$prix0_10}€→{$prix300plus}€/m² ✓ Livraison {$delai}j ✓ Qualité pro $certification ✓ Devis gratuit";
...
return <<<PHP
<?php
\$page_title = "$nom - Impression Grand Format | Prix Dégressifs | Imprixo";
\$page_description = "$metaDesc";
\$canonical_url = "https://imprixo.fr/produit/$code.php";
?>
...
<div class="bg-gradient-to-r from-red-600 to-red-500 text-white py-3 text-center">
    <p class="font-bold text-sm md:text-base">
        🔥 OFFRE LIMITÉE : Livraison GRATUITE dès 200€ + Devis sous 1h | Plus que <span class="pulse">$stock</span> en stock !
    </p>
</div>
